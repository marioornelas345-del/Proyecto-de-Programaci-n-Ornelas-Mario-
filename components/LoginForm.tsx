  ...
  useEffect(() => {
    ...
    if (urlError) {
      if (urlError === 'auth-callback-failed') {
        setError('Authentication failed during callback. This usually happens if cookies are disabled or the session expired.')
      } else if (urlError === 'access_denied') {
        setError('Access denied. You might have cancelled the login.')
      } else {
